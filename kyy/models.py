...
class GeometricScatterSSM(SequenceStateModel):
    ...
    Each token selects an independent angle per declared physical edge and sweep.
    Disjoint edges are updated in checkerboard phases, so global state mixing is
    generated by local geometry while recurrent work remains O(E)=O(state_dim).
    """
    ...
    def __init__(
        ...
        self.angle_raw = nn.Parameter(torch.empty(vocab_size, self.sweeps, self.n_edges))
    ...
    def _scatter_phase(
        ...
        theta = math.pi * torch.tanh(self.angle_raw[tok, sweep][:, edge_ids])
        c, s = torch.cos(theta), torch.sin(theta)
        a = h[:, src]
        b = h[:, dst]
        ap = c * a + s * b
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(length):
            ...
            for sweep in range(self.sweeps):
                h = self._scatter_phase(h, tok, sweep, self.phase0)
                h = self._scatter_phase(h, tok, sweep, self.phase1)
            outs.append(self.readout(h))
        return torch.stack(outs, dim=1)

    @torch.no_grad()
    def dense_transition(self, token: int) -> torch.Tensor:
        h = torch.eye(self.state_dim, device=self.angle_raw.device, dtype=self.angle_raw.dtype)
        tokv = torch.full((self.state_dim,), token, device=self.angle_raw.device, dtype=torch.long)
        for sweep in range(self.sweeps):
            h = self._scatter_phase(h, tokv, sweep, self.phase0)
            h = self._scatter_phase(h, tokv, sweep, self.phase1)
        return h.T
